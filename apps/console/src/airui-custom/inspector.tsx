  ...
  return (
    <div style={{ display: "grid", gap: 8, maxHeight: 260, overflowY: "auto", paddingRight: 2 }}>
      {skills.map((skill) => {
        ...
        return (
          <div key={skill.slug} className={running && on ? "skill-card skill-card-active" : "skill-card"} style={{ padding: 10, borderRadius: "var(--radius-md)", background: on ? "var(--color-primary-soft)" : "var(--color-surface-muted)", border: on ? "1px solid var(--color-primary-border)" : "1px solid transparent" }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: "var(--color-text)" }}>{skill.name}</div>
              {on && <span style={{ fontSize: 10, color: "var(--color-primary-strong)", fontWeight: 600, textTransform: "uppercase" }}>{String(resolvedProps.usingText ?? "")}</span>}
            </div>
...
export const RunTimeline: FC<{ comp: Component; resolvedProps: Record<string, unknown> }> = ({ resolvedProps }) => {
  ...
  return (
    <section className="timeline-dock" style={{ background: "var(--color-surface)", border: "1px solid var(--color-border)", borderRadius: "var(--radius-md)", overflow: "hidden" }}>
      ...
      <div style={{ display: "grid", gap: 6, maxHeight: 168, overflowY: "auto", padding: 9 }}>
        {events.slice().reverse().map((event) => (
          <div key={event.id} style={{ display: "grid", gridTemplateColumns: "56px 1fr", gap: 8, padding: "7px 8px", borderRadius: "var(--radius-sm)", background: "var(--color-surface-muted)", border: "1px solid var(--color-border)" }}>
